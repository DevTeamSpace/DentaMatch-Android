...
public class LoginActivity extends BaseActivity<LoginViewModel> implements View.OnClickListener {
    ...
    @Inject
    void injectViewModel(@NonNull LoginViewModel viewModel) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_preferred_job_location:
                if (mPreferredJobLocationDataArrayAdapter == null) {
                    mViewModel.getPreferredJobLocationList();
                    mViewModel.getPreferredLocationsList().observe(this, mLocationsListObserver);
                } else {
                    showLocationList();
                }
                break;
    ...
    private final Observer<LoginResponse> mSignUpObserver = this::onSignUpChanged;

    private void signUpApi() {
        mViewModel.signUp(getTextFromEditText(mBinder.registerEtEmail),
        ...
        mViewModel.getLogin().observe(this, mSignUpObserver);
    }
    ...
    private Observer<LoginResponse> mLoginObserver = this::onLoginChanged;

    private void signInApi() {
        mViewModel.signIn(getTextFromEditText(mBinder.loginEtEmail), getTextFromEditText(mBinder.loginEtPassword));
        mViewModel.getLogin().observe(this, mLoginObserver);
    }
    ...
    private void onLocationChanged(PreferredJobLocationModel preferredJobLocationModel) {
        mViewModel.getPreferredLocationsList().removeObserver(mLocationsListObserver);
        hideKeyboard();
        if (preferredJobLocationModel != null) {
            ArrayList<PreferredJobLocationData> mPreferredJobLocationList = (ArrayList<PreferredJobLocationData>) preferredJobLocationModel
                    .getResult()
    ...
    private void onSignUpChanged(LoginResponse response) {
        mViewModel.getLogin().removeObserver(mSignUpObserver);
    ...
    private void onLoginChanged(LoginResponse response) {
        mViewModel.getLogin().removeObserver(mLoginObserver);
        if (response != null && response.getStatus() == 1) {
